...
const YAHOO_AUTO_LOGIN_KEY = "yahoo_auto_login_attempted";
    ...
    async function loadBounds() {
      ...
      const oauthError =
        typeof window !== "undefined"
          ? new URLSearchParams(window.location.search).get("yahoo_error")
          : null;
      if (oauthError) {
        const url = new URL(window.location.href);
        url.searchParams.delete("yahoo_error");
        window.history.replaceState({}, "", url.pathname + url.search);
        setBoundsError(oauthError);
        return;
      }
      ...
      try {
        ...
        if (!res.ok || !body.ok) {
          const errBody = !body.ok ? body : null;
          const needsAuth = errBody?.code === "YAHOO_AUTH_REQUIRED" || res.status === 401;
          const alreadyTried =
            typeof window !== "undefined" &&
            sessionStorage.getItem(YAHOO_AUTO_LOGIN_KEY) === "1";

          // One auto-login per tab session when tokens are missing/rejected.
          // Do not auto-login on Yahoo Fantasy 403s — re-auth will not fix those and causes a loop.
          if (needsAuth && !alreadyTried && !attemptedAutoLoginRef.current) {
            attemptedAutoLoginRef.current = true;
            sessionStorage.setItem(YAHOO_AUTO_LOGIN_KEY, "1");
            window.location.href = "/api/auth/yahoo/login";
            return;
          }

          if (needsAuth && alreadyTried) {
            sessionStorage.removeItem(YAHOO_AUTO_LOGIN_KEY);
          }

          const detailParts = [
            errBody?.error ?? `HTTP ${res.status}`,
            errBody?.status ? `(Yahoo HTTP ${errBody.status})` : null,
            errBody?.leagueKey ? `league ${errBody.leagueKey}` : null,
            errBody?.details ? String(errBody.details).slice(0, 300) : null,
          ].filter(Boolean);
  ...
  if (boundsError) {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
        <p className="mb-3 whitespace-pre-wrap break-words">{boundsError}</p>
        <a
